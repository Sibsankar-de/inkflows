...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if ([email, userName, fullName].some((f) => f?.trim() === '')) {
  ...
  return res.status(200).json(new ApiResponse(200, createdUser, 'user Registered successfuly'));
});
...
const checkUserAuthenticated = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, { isAuthenticated }, 'Authentication checked'));
});
...
const updatePassword = asyncHandler(async (req, res) => {
  ...
  if (user?.authBy !== 'email')
    throw new ApiError(400, 'Only email authenticates can change password');
...
const getUserList = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, userList, 'User list fetched successfully'));
});
